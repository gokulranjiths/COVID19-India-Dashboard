...
fetch('https://api.covid19india.org/data.json', {mode: 'cors'})
  ...
  .then(function(text) {
    ...
    var cases_time_series = JSON.parse(text).cases_time_series;
    var date_arr=[];
    var daily_confirmed=[];
    var daily_deceased=[];
    var daily_recovered=[];
    ...
    cases_time_series.forEach(element => {
        // console.log(';loop', element.date);
        date_arr.push(element.date);
        daily_confirmed.push(element.dailyconfirmed);
        daily_deceased.push(element.dailydeceased);
        daily_recovered.push(element.dailyrecovered);
        total_confirmed.push(element.totalconfirmed);
        total_deceased.push(element.totaldeceased);
        total_recovered.push(element.totalrecovered);
   ...
   if ($('#coin_sales1').length) {
    ...
    var chart = new Chart(ctx, {
        ...
        data: {
            labels: date_arr.slice(1).slice(-10),
            datasets: [{
                label: "Cases",
                backgroundColor: "rgb(133, 168, 243)",
                borderColor: '#0b76b6',
                data: total_confirmed.slice(1).slice(-10),
            }]
        },
...
if ($('#coin_sales2').length) {
    ...
    var chart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: " New Cases",
                backgroundColor: "rgb(235, 207, 146)",
                borderColor: '#F0B41A',
                data: daily_confirmed.slice(1).slice(-10),
            }]
        },
        // Configuration options go here
        options: {
...
if ($('#coin_sales3').length) {
    ...
    var chart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: "Cases",
                backgroundColor: "rgb(154, 223, 170)",
                borderColor: '#69EC87',
                fill: true,
                data: total_recovered.slice(1).slice(-10),
            }]
        },
        // Configuration options go here
        options: {
...
if ($('#coin_sales4').length) {
    ...
    var chart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: "Cases",
                backgroundColor: "rgb(245, 198, 236)",
                borderColor: '#F16ED9',
                fill: true,
                data: total_deceased.slice(1).slice(-10),
            }]
        },
...
if ($('#verview-shart').length) {
    var myConfig = {
        ...
        "scale-x": { //X-Axis
            "labels": date_arr.slice(1).slice(-15),
            "label": {
                "font-size": 14,
                "offset-x": 0,
        ...
        "series": [{
                "values": total_confirmed.slice(1).slice(-15),
                "line-color": "#dc3545",
  ...
  fetch('https://api.covid19india.org/raw_data.json', {mode: 'cors'})
  .then(function(response) {
    return response.text();
  })
  .then(function(text) {
    ...
    var cases_time_series = JSON.parse(text).raw_data;
    var age_group=[];
    var p_gender=[];
    var current_status=[];
    ...
    cases_time_series.forEach(element => {
        // console.log(';loop', element.date);
        age_group.push(element.agebracket);
        p_gender.push(element.gender);
        current_status.push(element.currentstatus);
    ...
    var catg_6=[];
    var catg_o=[];
    for (i=0;i<age_group.length;i++){
        if(parseInt(age_group[i])>=0 && parseInt(age_group[i])<=20){
            catg_1.push(age_group[i]);
        }else if(parseInt(age_group[i])>20 && parseInt(age_group[i])<=30){
            catg_2.push(age_group[i]);
        }else if(parseInt(age_group[i])>30 && parseInt(age_group[i])<=50){
            catg_3.push(age_group[i]);
        }
        else if(parseInt(age_group[i])>50 && parseInt(age_group[i])<=80){
            catg_4.push(age_group[i]);
        }
        else if(parseInt(age_group[i])>80 && parseInt(age_group[i])<=100){
            catg_5.push(age_group[i]);
        }
        else if(parseInt(age_group[i])>100 ){
            catg_6.push(age_group[i]);
        }
        else {
            catg_o.push(age_group[i]);
        }

    }
    var catg1=catg_1.length;
    var catg2=catg_2.length;
    var catg3=catg_3.length;
    var catg4=catg_4.length;
    var catg5=catg_5.length;
    var catg6=catg_6.length;
    var catg7=catg_o.length;
...
function show_modal(state_name){
    // console.log(state_name);

    fetch('https://api.covid19india.org/v2/state_district_wise.json', {mode: 'cors'})
  .then(function(response) {
    return response.text();
  })
  .then(function(text) {
    ...
    for (i=0;i<data.length;i++){
        if(state_name==data[i]['state'].replace(/ +/g, "")){
            document.getElementById("modal_title_state").innerHTML =data[i]['state'] +" District Wise Confirmed Cases Count";
            var district_data=data[i]['districtData'];
            // console.log(district_data);
            var k= '<p>';
            for (j=0;j<district_data.length;j++){
                k+=district_data[j]['district']+" - "+district_data[j]['confirmed'];
                k+='<br>';


            }
            k+='</p>';
            document.getElementById('district_wise').innerHTML = k;
            break;
        }
        else{
            document.getElementById("modal_title_state").innerHTML = "No Data Available";
            document.getElementById('district_wise').innerHTML = "";
...
fetch('https://api.covid19india.org/data.json', {mode: 'cors'})
  ...
  .then(function(text) {
    ...
    var statewise = JSON.parse(text).statewise;

    var s_active=[];
    var s_confirmed=[];
    var s_deaths=[];
    var s_deltaconfirmed=[];
    var s_deltadeaths=[];
    var s_deltarecovered=[]
    var s_recovered=[];
    var s_state=[];
    ...
    statewise.forEach(element => {
        // console.log(';loop', element.date);
        s_active.push(element.active);
        s_confirmed.push(element.confirmed);
        s_deaths.push(element.deaths);
        s_deltaconfirmed.push(element.deltaconfirmed);
        s_deltadeaths.push(element.deltadeaths);
        s_deltarecovered.push(element.deltarecovered);
        s_recovered.push(element.recovered);
        s_state.push(element.state);
    ...
    // console.log('Request successful', text);
                    ...
                    var k = '<tbody>'
                    for(i = 1;i < s_state.length; i++){
                        var temp=s_state[i].replace(/ +/g, "");

                        k+= '<tr val="'+temp+'">';
                        k+= '<td>' + s_state[i] + '</td>';
                        k+= '<td>' + s_confirmed[i] + '</td>';
                        k+= '<td>' + s_active[i] + '</td>';
                        k+= '<td>' + s_recovered[i]+ '</td>';
                        k+= '<td>' + s_deaths[i]+ '</td>';
                        k+='<td><button type="button" class="btn btn-outline-primary" data-toggle="modal" data-target="#exampleLongModalLong2" onclick=show_modal("'+temp+'")>View</button></td>'
                        k+='<div class="modal fade" id="exampleLongModalLong2">';
                        k+='            <div class="modal-dialog">';
                        k+='                <div class="modal-content">';
                        k+='                    <div class="modal-header">';
                        k+='                        <h5 class="modal-title" id="modal_title_state"style=" color: #007bff; "></h5>';
                        k+='                        <button type="button" class="close" data-dismiss="modal"><span>&times;</span></button>';
                        k+='                    </div>';
                        k+='                    <div class="modal-body">';
                        k+='                        <p id="district_wise"></p>';
                        k+='                    </div>';
